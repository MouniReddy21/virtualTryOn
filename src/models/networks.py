...
class BaseNetwork(nn.Module):
    ...
    def print_network(self):
        ...
        print(
            "Network [{}] was created. Total number of parameters: {:.1f} million. "
            "To see the architecture, do print(network).".format(
                self.__class__.__name__, num_params / 1000000
            )
        )
    ...
    def init_weights(self, init_type="normal", gain=0.02):
        def init_func(m):
            ...
            elif ("Conv" in classname or "Linear" in classname) and hasattr(
                m, "weight"
            ):
                if init_type == "normal":
...
class TpsGridGen(nn.Module):
    def __init__(self, opt, dtype=torch.float):
        ...
        grid_X = (
            torch.tensor(grid_X, dtype=dtype).unsqueeze(0).unsqueeze(3)
        )  # size: (1, h, w, 1)
        grid_Y = (
            torch.tensor(grid_Y, dtype=dtype).unsqueeze(0).unsqueeze(3)
        )  # size: (1, h, w, 1)
...
class ALIASNorm(nn.Module):
    def __init__(self, norm_type, norm_nc, label_nc):
        ...
        if param_free_norm_type == "batch":
            ...
            raise ValueError(
                "'{}' is not a recognized parameter-free normalization type in ALIASNorm".format(
...
class ALIASGenerator(BaseNetwork):
    def __init__(self, opt, input_nc):
        ...
        for i in range(1, 8):
            self.add_module(
                "conv_{}".format(i), nn.Conv2d(input_nc, 16, kernel_size=3, padding=1)
            )
    ...
    def compute_latent_vector_size(self, opt):
        if self.num_upsampling_layers == "normal":
            ...
            raise ValueError(
                "opt.num_upsampling_layers '{}' is not recognized".format(
    ...
    def forward(self, x, seg, seg_div, misalign_mask):
        samples = [
            F.interpolate(x, size=(self.sh * 2**i, self.sw * 2**i), mode="nearest")
            for i in range(8)
        ]
